import { createFileRoute, Link } from "@tanstack/react-router";
import { useEffect, useState } from "react";
import { zodValidator, fallback } from "@tanstack/zod-adapter";
import { z } from "zod";
import { LogIn, UserPlus } from "lucide-react";
import { Alert, AlertDescription } from "@/components/ui/alert";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { signInWithEmail, signInWithOAuth, signUpWithEmail } from "@/integrations/supabase/auth";
import { useAuth } from "@/lib/auth";
import { toast } from "sonner";

const authSearchSchema = z.object({
  redirect: fallback(z.string().optional(), undefined),
  error: fallback(z.string().optional(), undefined),
});

function normalizeRedirect(redirect?: string) {
  return redirect?.startsWith("/") ? redirect : "/dashboard";
}

function normalizePhone(phone: string) {
  return phone.replace(/[^\d+\-\s()]/g, "").trim();
}

function isValidPhone(phone: string) {
  const digitCount = phone.replace(/\D/g, "").length;
  return digitCount >= 10 && digitCount <= 15;
}

export const Route = createFileRoute("/auth")({
  validateSearch: zodValidator(authSearchSchema),
  head: () => ({
    meta: [{ title: "Login or Sign up — Purulia Properties" }],
  }),
  component: AuthPage,
});

function AuthPage() {
  const { user } = useAuth();
  const search = Route.useSearch();
  const [tab, setTab] = useState<"login" | "signup">("login");
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");
  const [loading, setLoading] = useState(false);
  const [submittedAuth, setSubmittedAuth] = useState(false);
  const [successBurst, setSuccessBurst] = useState(false);
  const [formMessage, setFormMessage] = useState<{
    type: "error" | "success";
    text: string;
  } | null>(null);
  const redirectTo = normalizeRedirect(search.redirect);

  const getAuthErrorMessage = (message?: string) => {
    if (!message) return "Something went wrong. Please try again.";
    const normalized = message.toLowerCase();
    if (normalized.includes("rate limit") || normalized.includes("too many requests")) {
      return "Too many attempts. Please wait a few minutes and try again.";
    }
    if (normalized.includes("invalid login credentials")) {
      return "Invalid email or password.";
    }
    if (
      normalized.includes("user already registered") ||
      normalized.includes("already registered") ||
      normalized.includes("already exists") ||
      normalized.includes("email already")
    ) {
      return "This email is already registered. Please log in instead.";
    }
    return message;
  };

  const showAuthMessage = (type: "error" | "success", text: string) => {
    setFormMessage({ type, text });
    if (type === "error") toast.error(text);
    else toast.success(text);
  };

  const showSuccessBurst = (message: string, afterBurst?: () => void) => {
    showAuthMessage("success", message);
    setSuccessBurst(true);
    window.setTimeout(() => {
      setSuccessBurst(false);
      afterBurst?.();
    }, 1350);
  };

  useEffect(() => {
    if (user && !submittedAuth) {
      window.location.assign(redirectTo);
    }
  }, [redirectTo, submittedAuth, user]);

  const onLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setFormMessage(null);
    setSubmittedAuth(true);
    setLoading(true);
    const { error } = await signInWithEmail({ email, password });
    setLoading(false);
    if (error) {
      setSubmittedAuth(false);
      showAuthMessage("error", getAuthErrorMessage(error.message));
    } else {
      showSuccessBurst("Welcome back!", () => window.location.assign(redirectTo));
    }
  };

  const onSignup = async (e: React.FormEvent) => {
    e.preventDefault();
    setFormMessage(null);
    setSubmittedAuth(true);
    const trimmedName = name.trim();
    const normalizedPhone = normalizePhone(phone);
    if (!trimmedName) {
      setSubmittedAuth(false);
      showAuthMessage("error", "Please enter your full name.");
      return;
    }
    if (!isValidPhone(normalizedPhone)) {
      setSubmittedAuth(false);
      showAuthMessage("error", "Please enter a valid mobile number.");
      return;
    }
    setLoading(true);
    const { data, error } = await signUpWithEmail({
      email: email.trim(),
      password,
      displayName: trimmedName,
      phone: normalizedPhone,
    });
    setLoading(false);
    const identities = data.user?.identities;
    const isExistingUser = Array.isArray(identities) && identities.length === 0;
    if (error) {
      setSubmittedAuth(false);
      showAuthMessage("error", getAuthErrorMessage(error.message));
    } else if (isExistingUser) {
      setSubmittedAuth(false);
      showAuthMessage("error", "This email is already registered. Please log in instead.");
      setTab("login");
      setPassword("");
    } else {
      showSuccessBurst("Account created successfully. Please log in to continue.", () => {
        setSubmittedAuth(false);
        setTab("login");
        setName("");
        setPhone("");
        setPassword("");
      });
    }
  };

  const onGoogle = async () => {
    setFormMessage(null);
    const { error } = await signInWithOAuth("google", redirectTo);
    if (error)
      showAuthMessage("error", getAuthErrorMessage(error.message) || "Google sign-in failed");
  };

  return (
    <div className="container mx-auto flex min-h-[70vh] items-center justify-center px-4 py-12 md:px-6">
      <Card className="auth-neon-card relative isolate w-full max-w-md overflow-hidden p-6 md:p-8">
        {successBurst ? (
          <div className="auth-success-ribbon" aria-hidden="true">
            <span />
            <span />
            <span />
            <span />
            <span />
            <span />
          </div>
        ) : null}

        <div className="text-center">
          <h1 className="font-display text-2xl font-bold">Welcome to Purulia Properties</h1>
          <p className="mt-1 text-sm text-muted-foreground">Login or create your free account</p>
        </div>

        <Tabs value={tab} onValueChange={(v) => setTab(v as "login" | "signup")} className="mt-6">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="login">
              <LogIn className="h-4 w-4" /> Login
            </TabsTrigger>
            <TabsTrigger value="signup">
              <UserPlus className="h-4 w-4" /> Sign up
            </TabsTrigger>
          </TabsList>

          {formMessage ? (
            <Alert
              variant={formMessage.type === "error" ? "destructive" : "default"}
              className="mt-4"
            >
              <AlertDescription>{formMessage.text}</AlertDescription>
            </Alert>
          ) : null}

          <TabsContent value="login">
            <form onSubmit={onLogin} className="mt-4 space-y-3">
              <div>
                <Label htmlFor="le">Email</Label>
                <Input
                  id="le"
                  type="email"
                  autoComplete="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  required
                  className="mt-1"
                />
              </div>
              <div>
                <Label htmlFor="lp">Password</Label>
                <Input
                  id="lp"
                  type="password"
                  autoComplete="current-password"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  required
                  className="mt-1"
                />
              </div>
              <Button type="submit" disabled={loading} className="w-full">
                Login
              </Button>
            </form>
          </TabsContent>

          <TabsContent value="signup">
            <form onSubmit={onSignup} className="mt-4 space-y-3">
              <div>
                <Label htmlFor="sn">Full name</Label>
                <Input
                  id="sn"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  required
                  className="mt-1"
                  autoComplete="name"
                  maxLength={80}
                />
              </div>
              <div>
                <Label htmlFor="sph">Mobile number</Label>
                <Input
                  id="sph"
                  type="tel"
                  autoComplete="tel"
                  value={phone}
                  onChange={(e) => setPhone(e.target.value)}
                  required
                  className="mt-1"
                  maxLength={20}
                  placeholder="Enter your mobile number"
                />
              </div>
              <div>
                <Label htmlFor="se">Email</Label>
                <Input
                  id="se"
                  type="email"
                  autoComplete="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  required
                  className="mt-1"
                />
              </div>
              <div>
                <Label htmlFor="sp">Password (min 8 chars)</Label>
                <Input
                  id="sp"
                  type="password"
                  autoComplete="new-password"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  required
                  minLength={8}
                  className="mt-1"
                />
              </div>
              <Button
                type="submit"
                disabled={loading}
                className="w-full bg-orange text-orange-foreground hover:bg-orange/90"
              >
                Create account
              </Button>
            </form>
          </TabsContent>
        </Tabs>

        <div className="my-5 flex items-center gap-3 text-xs text-muted-foreground">
          <div className="h-px flex-1 bg-border" />
          OR
          <div className="h-px flex-1 bg-border" />
        </div>

        <Button type="button" variant="outline" className="w-full" onClick={onGoogle}>
          <svg className="h-4 w-4" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.99.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84C6.71 7.31 9.14 5.38 12 5.38z"
            />
          </svg>
          Continue with Google
        </Button>

        <p className="mt-4 text-center text-xs text-muted-foreground">
          By continuing you agree to our terms.{" "}
          <Link to="/" className="text-primary hover:underline">
            Back to home
          </Link>
        </p>
      </Card>
    </div>
  );
}
